...
Save the PRD into `c:\Users\user\Documents\projects\efootball\.zencoder\chats\73f0d6af-abc1-4f5e-8e97-b20ea526f0d4/requirements.md`.

### [x] Step: Technical Specification

Based on the PRD in `c:\Users\user\Documents\projects\efootball\.zencoder\chats\73f0d6af-abc1-4f5e-8e97-b20ea526f0d4/requirements.md`, create a detailed technical specification to be used by a coding agent to implement the feature. Follow this template:
...
Save the spec to `c:\Users\user\Documents\projects\efootball\.zencoder\chats\73f0d6af-abc1-4f5e-8e97-b20ea526f0d4/spec.md`.

### [x] Step: Implementation Plan

Based on the technical spec in `c:\Users\user\Documents\projects\efootball\.zencoder\chats\73f0d6af-abc1-4f5e-8e97-b20ea526f0d4/spec.md`, create a detailed task plan and update `c:\Users\user\Documents\projects\efootball\.zencoder\chats\73f0d6af-abc1-4f5e-8e97-b20ea526f0d4/plan.md`. Each task should have task definition, references to contracts to be used/implemented, deliverable definition and verification instructions.
...
### [ ] Step: Set up Centralized API Config
Create `frontend/src/config/api.ts` that exports `API_BASE_URL` using `import.meta.env.VITE_API_URL` with a fallback to the local backend.

### [ ] Step: Refactor Hardcoded URLs
Replace all occurrences of `https://efootball-3.onrender.com` with `API_BASE_URL` across the frontend codebase.

### [ ] Step: Configure Vercel SPA Routing
Ensure `vercel.json` is correctly placed in the `frontend` directory and contains the necessary rewrite rules for SPA support.

### [ ] Step: Add Admin Link to Header
Add a link to the admin login page in the `Header` component for easier access.

### [ ] Step: Final Verification and Cleanup
Verify that the admin page is accessible, login works, and data is fetched correctly. Run lint and typecheck.
